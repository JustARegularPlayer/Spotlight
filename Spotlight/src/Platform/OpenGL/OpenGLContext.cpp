...
namespace Spotlight
{
								 ...
								 const void *userParam)
	{
		...
		SPL_CORE_FATAL("Source = {}", GLErrorToString(source));
		SPL_CORE_FATAL("Type = {}", GLErrorToString(type));
		SPL_CORE_FATAL("ID = {}", id);
		SPL_CORE_FATAL("Severity = {}", GLErrorToString(severity));
		SPL_CORE_FATAL("Message:\n {}", message);

		if(severity == GL_DEBUG_SEVERITY_HIGH)
			SPL_CORE_ASSERT(false, '\0');
	}
